...
import generateErrorUtil from '../../helpers/generateError.js'; // Importa la función para generar errores
...
const resetUserPassController = async (req, res, next) => {
    ...
    try {
        ...
        if (!newPassword || !repeatNewPassword) {
            generateErrorUtil('No se ha introducido ninguna contraseña', 400);
        }
        ...
        const [user] = await pool.query('SELECT * FROM users WHERE recoverPassCode = ?', [recoverPassCode]);

        // Si no se encuentra ningún usuario, generamos un error
        if(user.length < 1){
            generateErrorUtil('No se ha encontrado ningún usuario con ese código de recuperación', 404);
        }
        ...
        await pool.query('UPDATE users SET password = ?, recoverPassCode = NULL WHERE recoverPassCode = ?', [hashedPassword, recoverPassCode]);
